...
Business logic library with Zustand stores for the Entity_Starter application.

**npm**: `@sudobility/entityentitystarter_lib` (restricted, BUSL-1.1)
...
Unified hook that combines entityentitystarter_client hooks + Zustand store + business logic:
...
This is the primary hook consumed by UI layers (entityentitystarter_app, entityentityentitystarter_app_rn).
...
```
entityentitystarter_app / entityentityentitystarter_app_rn
    ↓ uses
@sudobility/entityentitystarter_lib (this package)
    ↓ uses
@sudobility/entityentitystarter_client (API hooks)
    ↓ uses
@sudobility/entityentitystarter_types (type definitions)
```

## Related Projects

- **entityentitystarter_types** — Shared type definitions; imported transitively via entityentitystarter_client
- **entityentitystarter_client** — API client SDK; this library wraps its hooks with business logic and Zustand state
- **entityentitystarter_app** — Web frontend that consumes `useHistoriesManager` from this library
- **entityentityentitystarter_app_rn** — React Native app that consumes `useHistoriesManager` via file: links
- **entityentitystarter_api** — Backend server; this library communicates with it indirectly through entityentitystarter_client

## Coding Patterns

- `useHistoriesManager` is the primary hook -- it orchestrates entityentitystarter_client hooks + Zustand store into a single unified interface for UI layers
...
- This is a published npm package (`@sudobility/entityentitystarter_lib`) -- coordinate breaking changes with entityentitystarter_app and entityentityentitystarter_app_rn
